...
const CONTACTS_TABLE_ROW_HEIGHT_PX = 60
...
export default function ContatosPage() {
  ...
  const [isEditingSelectedContact, setIsEditingSelectedContact] =
    useState(false)
  ...
  const [confirmingDeleteContactId, setConfirmingDeleteContactId] = useState<
    string | null
  >(null)
  ...
  const sortedFilteredContacts = [...filteredContacts].sort(
    (firstContact, secondContact) => {
      ...
      return (
        getSortValue(firstContact).localeCompare(
          getSortValue(secondContact),
          'pt-BR',
          {
  ...
  const isContactFormReadOnly =
    isViewingSelectedContact && !isEditingSelectedContact
    ...
    try {
      ...
      if (isEditingSelectedContact && selectedContact) {
        ...
        const createdContact =
          await ContactsService.createContact(contactPayload)
    ...
    >
          ...
          {!isViewingSelectedContact ? (
            <button
              ...
              style={{
                ...
                borderRadius: 6,
                background: 'transparent',
                color: canCreateContact ? '#6b7280' : '#cbd5e1',
                display: 'inline-flex',
                alignItems: 'center',
                justifyContent: 'center',
                padding: 0,
                cursor: canCreateContact ? 'pointer' : 'not-allowed',
              }}
            >
              <Save size={18} />
            </button>
      ...
      {isViewingSelectedContact && isConfirmingPanelDelete ? (
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <button
              type="button"
              aria-label="Cancelar exclusão do contato aberto"
              onClick={() => setIsConfirmingPanelDelete(false)}
            ...
            <button
              type="button"
              aria-label="Confirmar exclusão do contato aberto"
              onClick={() =>
                selectedContact && void handleDeleteContact(selectedContact.id)
              }
              style={actionButtonStyle}
            >
              ✓
      ...
      >
        <label style={{ display: 'grid', gap: 8 }}>
          <span style={contactFieldLabelStyle}>Nome</span>
          <input
            type="text"
            value={name}
        ...
        <label style={{ display: 'grid', gap: 8 }}>
          <span style={contactFieldLabelStyle}>Telefone</span>
          <input
            type="text"
            value={phone}
            onChange={(event) =>
              setPhone(formatLeadPhoneInput(event.target.value))
            }
        ...
        <label style={{ display: 'grid', gap: 8 }}>
          <span style={contactFieldLabelStyle}>Empresa</span>
          <input
            type="text"
            value={company}
        ...
        <label style={{ display: 'grid', gap: 8 }}>
          <span style={contactFieldLabelStyle}>Instagram</span>
          <div style={{ position: 'relative' }}>
            ...
            <input
              ...
              value={instagram}
              onChange={(event) =>
                setInstagram(formatInstagramHandle(event.target.value))
              }
  ...
  const selectedContactPanel =
    isViewingSelectedContact && isMobile ? (
      <aside
        style={{
          position: 'absolute',
          inset: 0,
                  ...
                  {confirmingDeleteContactId === contact.id ? (
                    <>
                      ...
                      <div style={{ display: 'flex', gap: 8 }}>
                        <button
                          type="button"
                          aria-label="Cancelar exclusão de contato"
                        ...
                        <button
                          type="button"
                          aria-label="Confirmar exclusão de contato"
                    ...
                    <div style={{ minWidth: 0, flex: 1 }}>
                      ...
                      {contact.instagram ? (
                        <div style={{ marginTop: 8 }}>
                          <InstagramTag value={contact.instagram} />
                        </div>
                      ) : null}
                    </div>
                  )}
            ...
            >
              ...
              >
                <button
                  ...
                  style={getHeaderSortButtonStyle('phone')}
                >
                  Telefone{' '}
                  <span style={{ fontSize: 11 }}>
              ...
              >
                <button
                  ...
                  style={getHeaderSortButtonStyle('company')}
                >
                  Empresa{' '}
                  <span style={{ fontSize: 11 }}>
              ...
              >
                <button
                  type="button"
                  onClick={() => handleSortToggle('instagram')}
                  style={getHeaderSortButtonStyle('instagram')}
                >
                  Instagram{' '}
                  <span style={{ fontSize: 11 }}>
                    {getSortIndicator('instagram')}
                  </span>
                </button>
              </th>
              <th
            ...
            {!isLoading && sortedFilteredContacts.length > 0
              ? sortedFilteredContacts.map((contact) => (
                  <tr
                    ...
                    style={{
                      height: CONTACTS_TABLE_ROW_HEIGHT_PX,
