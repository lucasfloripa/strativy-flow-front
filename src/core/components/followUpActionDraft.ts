...
export const fromFollowUpActionPayload = (
  action?: FollowUpActionPayload | null,
): FollowUpActionDraft => {
  if (!action) {
    return {
      ...initialFollowUpActionDraft,
...
export const isFollowUpActionDraftValid = (
  ...
  if (draft.type === 'send_message') {
    if (draft.channel === 'whatsapp') {
      ...
      return Boolean(
        isLeadPhoneComplete(draft.phone) &&
        draft.templateId &&
        !hasMissingVariable,
      )
    }
...
export const toFollowUpActionPayload = (
  draft: FollowUpActionDraft,
): FollowUpActionPayload => {
  if (draft.type === 'agenda') {
    return {
      ...
      return {
        ...
        payload: {
          ...
          variables: Object.fromEntries(
            Object.entries(draft.templateVariables).filter(([, value]) =>
              value.trim(),
            ),
          ),
        },
